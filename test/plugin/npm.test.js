...
describe("local", () => {
  test("no-package", async () => {
    const error = await returnError(local, ["npm"]);
    expect(error.message).toBeDefined();
    expect(
      error.message.startsWith(
        "Could not find project file package.json for project type npm",
      ),
    ).toBeTruthy();
  });
  ...
  describe("with-package", () => {
    beforeEach(async () => {
      await setupNpm(execOpts);
    });

    test("projectType=npm", async () => {
      let version = await local("npm");
...
describe("update", () => {
  ...
  test("no-commit", async () => {
    ...
    const error = await returnError(update, ["npm"]);
    expect(error.message).toBeDefined();
