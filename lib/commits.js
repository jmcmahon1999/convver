...
const checkCommits = (messages, commitTypes) => {
  ...
  let headCommit = commits[0];
  if (
    !headCommit.type &&
    (headCommit.description.startsWith("Merge") ||
      headCommit.description.startsWith("Revert"))
  )
    headCommit = commits[1];
  if (!headCommit.type)
    throw new Error(`Unconventional commit ${headCommit.description} at HEAD.`);
  if (commitTypes["unsafe"].includes(headCommit.type) || !headCommit.type)
    throw new Error(`Unsafe commit type ${headCommit.type} at HEAD.`);
  return commits;
};
